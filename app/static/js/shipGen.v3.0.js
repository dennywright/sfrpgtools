...
function generateShip() {
  ...
  while (thrusterArray.length == 0) {
    var tier = $("#tierPicker")
      ...
      .trim()
      .replace("Tier ", "")
      .replace("Any tier", Object.keys(shipTiers).selectRandom());
    var frame = $("#framePicker")
      .val()
      .trim()
      .replace("Any frame", Object.keys(shipFrames).selectRandom());
    var weapons = $("#weaponPicker")
      .val()
      .trim()
      .replace(" armed", "")
      .replace("Any armament", ["Not", "Lightly", "Heavily"].selectRandom());
    ...
    //THRUSTERS
    var thrusterArray = getThrusters(
      frameObj.size,
      buildPoints,
      powerCoreUnits
    );
    ...
    if (thrusterArray.length == 0 && !$("#tierPicker").val().includes("Any")) {
      if (!$("#framePicker").val().includes("Any")) {
        clearOutput();
        $outputArea.append(
          "<p>This Tier and Frame combo will not work. Not enough Build Points.</p>"
        );
        return;
      }
    }
      ...
      for (weaponClass in mounts[mountList[i]]) {
        ...
        if (buildPoints > 0) {
          ...
          for (var j = 0; j < randWeapons; j++) {
            var weaponArray = getWeapons(
              weaponClass.capitalise(),
              buildPoints,
              powerCoreUnits
            );
            if (weaponArray.length > 0) {
              var weapon = weaponArray.selectRandom();
              shipBlock.mounts[mountList[i]].push(
                weapon + " (" + shipWeapons[weapon].damage + ")"
              );
  ...
  shipBlock.AC = 10 + sizeMod;
  shipBlock.TL = 10 + sizeMod;
  shipBlock.systems = [];
  shipBlock.modifiers = [];

  if (shipBlock.piloting != 0) {
    shipBlock.modifiers.push(
      ("+" + shipBlock.piloting.toString() + " Piloting").replace("+-", "-")
    );
  }
  ...
  for (var i = 0; i < essentialSystems.length; i++) {
    if (essentialSystems[i] == "shipArmor") {
      ...
      if (computer != "Basic") {
        var modstring =
          "+" +
          shipComputers[computer].value.computerBonus +
          " any " +
          shipComputers[computer].value.computerNodes
            .toString()
            .replace("1", "one check")
            .replace("2", "two checks")
            .replace("3", "three checks")
            .replace("4", "four checks") +
          " per round";
  ...
  //OTHER SYSTEMS
  var otherSystems = shuffle([
    "shipDriftEngines",
    "shipSensors",
    "shipQuarters",
    "shipExpansionBays",
  ]); //,"shipExpansionBays","shipSecurity"
  for (var i = 0; i < otherSystems.length; i++) {
    if (otherSystems[i] == "shipDriftEngines") {
      var driftArray = getDriftEngine(
        frameObj.size,
        buildPoints,
        shipBlock.PCU
      );
      ...
      if (driftArray.length > 0) {
        ...
        shipBlock.driftRating =
          shipDriftEngines[engine].value.driftEngineRating;
        buildPoints -=
          shipDriftEngines[engine].BPCostMultiplier * frameObj.size;
      }
      ...
      if (sensorArray.length > 0) {
        ...
        if (shipSensors[sensor].sensorMod != "+0") {
          shipBlock.modifiers.push(
            shipSensors[sensor].sensorMod + " Computers"
          );
        }
      } else {
      ...
      for (var j = 0; j < remainingBays; j++) {
        ...
        if (bayArray.length > 0) {
          ...
          if (baySelections.includes(bay)) {
            bayNumbers[baySelections.indexOf(bay)] = bayNumbers[
              baySelections.indexOf(bay)
            ] += 1;
          } else {
      ...
      if (bayNumbers.length > 0) {
        var baySum = bayNumbers.reduce(function (acc, val) {
          return acc + val;
        });
      } else {
        var baySum = 0;
      ...
      if (baySelections.length > 0) {
        for (var k = 0; k < baySelections.length; k++) {
          ...
          if (bayNumbers[k] > 1) {
            ...
            bayString = bayString
              .replace("boatss", "boats")
              .replace("seatings", "seating");
          }
...
function displayShipBlock(shipBlock) {
  ...
  textBlock +=
    '<div style="padding: 10px 0px 10px 0px;"><i>This</i> ' +
    shipBlock.size.toLowerCase() +
    " " +
    shipBlock.frame.toLowerCase() +
    " <i>" +
    flavorText.selectRandom().toLowerCase() +
    "</i></div>";
  textBlock +=
    "<div>" +
    "<b>Speed</b> " +
    ...
    "; " +
    "<b>Maneuverability</b> " +
    shipBlock.maneuverability.toLowerCase() +
    " (turn " +
    shipBlock.turn +
    ")";
  ...
  if (shipBlock.mounts.ForwardArc.length > 0) {
    textBlock +=
      "<div>" +
      "<b>Attack (Forward)</b> " +
      shipBlock.mounts.ForwardArc.join(", ") +
      "</div>";
  }
  if (shipBlock.mounts.PortArc.length > 0) {
    textBlock +=
      "<div>" +
      "<b>Attack (Port)</b> " +
      shipBlock.mounts.PortArc.join(", ") +
      "</div>";
  }
  if (shipBlock.mounts.StarboardArc.length > 0) {
    textBlock +=
      "<div>" +
      "<b>Attack (Starboard)</b> " +
      shipBlock.mounts.StarboardArc.join(", ") +
      "</div>";
  }
...
function leftAndRight(left, right) {
  return (
    ...
    left +
    '</div></div><div class="col-xs-4"><div class="text-right">' +
    right +
    "</div></div></div>"
  );
}
...
function getCores(size, buildPoints) {
  ...
  for (core in shipPowerCores) {
    if (
      size <= shipPowerCores[core].maxSize &&
      size >= shipPowerCores[core].minSize &&
      shipPowerCores[core].cost.BP <= buildPoints
    ) {
      cores.push(core);
    }
  }
...
function getThrusters(size, buildPoints, powerCoreUnits) {
  ...
  for (thruster in shipThrusters) {
    if (
      size == shipThrusters[thruster].size &&
      shipThrusters[thruster].cost.BP <= buildPoints &&
      shipThrusters[thruster].cost.PCU <= powerCoreUnits
    ) {
...
function getWeapons(weaponClass, buildPoints, powerCoreUnits) {
  ...
  for (weapon in shipWeapons) {
    if (
      weaponClass == shipWeapons[weapon].class &&
      shipWeapons[weapon].cost.BP <= buildPoints &&
      shipWeapons[weapon].cost.PCU <= powerCoreUnits
    ) {
      weapons.push(weapon);
    }
  }
...
function getComputers(buildPoints, powerCoreUnits) {
  ...
  for (computer in shipComputers) {
    if (
      shipComputers[computer].cost.BP <= buildPoints &&
      shipComputers[computer].cost.PCU <= powerCoreUnits
    ) {
...
function getShields(buildPoints, powerCoreUnits) {
  ...
  for (shield in shipShields) {
    if (
      shipShields[shield].cost.BP <= buildPoints &&
      shipShields[shield].cost.PCU <= powerCoreUnits
    ) {
      shields.push(shield);
    }
  }
...
function getDefenses(buildPoints, powerCoreUnits) {
  ...
  for (defense in shipDefenses) {
    if (
      shipDefenses[defense].cost.BP <= buildPoints &&
      shipDefenses[defense].cost.PCU <= powerCoreUnits
    ) {
...
function getDriftEngine(size, buildPoints, power) {
  ...
  for (engine in shipDriftEngines) {
    if (
      shipDriftEngines[engine].BPCostMultiplier * size <= buildPoints &&
      size <= shipDriftEngines[engine].maxSize &&
      power >= shipDriftEngines[engine].minPCU
    ) {
      engines.push(engine);
    }
  }
...
function getExpansionBays(size, buildPoints, powerCoreUnits, slots) {
  ...
  for (bay in shipExpansionBays) {
    if (
      size >= shipExpansionBays[bay].minSize &&
      shipExpansionBays[bay].cost.BP <= buildPoints &&
      shipExpansionBays[bay].cost.PCU <= powerCoreUnits &&
      shipExpansionBays[bay].slots <= slots &&
      bay != "Cargo hold" &&
      bay != "Power core housing"
    ) {
      bays.push(bay);
    }
  }
...
function generateName(type) {
  ...
  var name =
    shipNames.Word.selectRandom() + " " + shipNames.Words.selectRandom();
  if (classShip != type) {
    name += " " + classShip;
  } else {
    name = "The " + name;
